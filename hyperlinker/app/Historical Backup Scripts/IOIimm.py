    ...
    if request.method == 'POST':
        ...
        def HRA_Case_Coding(LPC,SLPC,HRA_LoS,Crim):
            ...
            if HRA_LoS == '***Needs Cleanup***':
                ...
                return 'B -OTH_Emergency Planning'    
                ...
                return 'T1-OTH_I-912'
            elif SLPC == "I-129F" or SLPC == "I-130" or SLPC == "I-485 Affirmative" or SLPC == "I-751" or SLPC == "I-864"or SLPC == "I-864EZ" or SLPC == "AOS I-130":
                return 'T1-FAM'
            elif SLPC == "204(L)":
                return 'T2-HO_204(L)'
            elif SLPC == "AR-11":
                ...
                return 'T1-OTH_DS-160'
            elif SLPC == "DS-260":
                return 'T1-OTH_DS-260'   
            elif SLPC == "EOIR 27":
                return 'T2-REPO_EOIR27'    
            elif SLPC == "EOIR-26" or SLPC == "EOIR-29 BIA Appeal" or SLPC.startswith("I-290B") == True or SLPC == "N-336":
                return 'T2-APO'
            elif SLPC == "EOIR-26A":
        ...
        def Eligibility_Date(Effective_Date,Date_Opened):
            if Effective_Date != '':
                ...
                return Date_Opened
        data_xls['Eligibility Date'] = data_xls.apply(lambda x : Eligibility_Date(x['IOI HRA Effective Date (optional) (IOI 2)'],x['Date Opened']), axis = 1)
        
        #Manipulable Dates               
        
        data_xls['Open Month'] = data_xls['Eligibility Date'].apply(lambda x: str(x)[:2])
        data_xls['Open Day'] = data_xls['Eligibility Date'].apply(lambda x: str(x)[3:5])
        data_xls['Open Year'] = data_xls['Eligibility Date'].apply(lambda x: str(x)[6:])
        ...
        def DHCI_Needed(DHCI,Open_Construct):
            if Open_Construct == '':
                return ''
        ...
        data_xls['Needs DHCI?'] = data_xls.apply(lambda x: DHCI_Needed(x['Has Declaration of Household Composition and Income (DHCI) Form?'],x['Open Construct']), axis=1)
        
        
        #Needs Substantial Activity to Rollover into FY'20
        
        def Needs_Rollover(Open_Construct,Substantial_Activity,Substantial_Activity_Date):
        ...
        data_xls['Needs Substantial Activity?'] = data_xls.apply(lambda x: Needs_Rollover(x['Open Construct'],x['Substantial Activity'],x['Subs Construct']), axis=1)
        
        #is case's eligibility date in FY'20?
        
        #if not - does it have a substantial activity AND a substantial activity date in FY'20?
        
        #if not - tell them that we need substantial activity performed
        ...
        #Marking non-removal cases as tier 2 minor removal, as appropriate
        
        #make client identifier
        
        #identify duplicates
        
        #if a client has multiple entries and one of the entries has "Tier 2 (minor removal)", make the rest of them ALSO have "Tier 2 (minor removal)"
        
        
                
        #Put everything in the right order
        data_xls = data_xls[['Hyperlinked Case #','Office','Primary Advocate','Client Name','Special Legal Problem Code','Level of Service','Country of Origin','Consent form?','Exclude due to Income?','Needs DHCI?','Needs Substantial Activity?','Deliverable Tally']]      
