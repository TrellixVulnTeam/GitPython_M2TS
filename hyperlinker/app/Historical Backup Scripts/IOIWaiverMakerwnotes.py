...
def IOIWaiverMaker():
    ...
    if request.method == 'POST':
        ...
        '''#Function for proceeding type replacement (functions)
        def ethingWrong(ProceedingType):
            if ProceedingType == 'ething is wrong':
                return 'Needs Cleanup'
            else:
                return ProceedingType'''
        #str.replace('ething is wrong','Something is wrong') - didn't work, string not defined
        #df['Proceeding Type']=df.apply(lambda x: "Som"+ x['Proceeding Type'] if x == 'ething is wrong'-doesn't work, affects df throughout script
        #df['age']=df.apply(lambda x: x['age']+3,axis=1)-basis for above attempt
        #df['Proceeding Type'] = df['Proceeding Type'].apply(lambda x: str(x) if x != 'ething is wrong' else str('Needs Cleanup'))
        df['Proceeding Type'] = df['Proceeding Type'].replace('ething is wrong', 'Needs Cleanup')
                
        #df['Proceeding Type'] = df.apply(lambda x: ethingWrong(x['Proceeding Type']), axis=1)
        
        #if df['Proceeding Type'] == 'ething is wrong': return 'Something is wrong'. Doesn't work because asking about entire column, not ind variables
        ...
        if request.form.get('Immigration'):
            #worksheet.conditional_format(GRowRange,{'type': 'cell','criteria': '==','value': '"ething is wrong"','format': problem_format})
            worksheet.conditional_format(GRowRange,{'type': 'cell','criteria': '==','value': '"Needs Cleanup"','format': problem_format})
