...
def ComplianceConsolidater():
    ...
    if request.method == 'POST':
        ...
        def TesterTester(CitImmTester,RetainerTester):
            if CitImmTester != "" or RetainerTester != "":
                return "Needs Review"
            else:
                return ""
        df['Tester Tester'] = df.apply(lambda x : TesterTester(x['Citizenship & Immigration Tester'],x['Retainer Tester']),axis=1)        
        
        """
        #check for just retainer or citizenship
        def RetainCitizen(CitizenTester,RetainerTester):
            if RetainerTester != '' or CitizenTester != '':
                return "May Need In-Office Review"
            else:
                return ''
        
        df['Retainer & Citizen Tester'] = df.apply(lambda x : RetainCitizen(x['Citizenship & Immigration Tester'],x['Retainer Tester']),axis=1)
        """
        ...
        df = df[['Hyperlinked CaseID#','Date Opened','Primary Advocate Name','Client First Name','Client Last Name','No Legal Assistance Documented Tester','No Time Entered for 90 Days Tester','200% of Poverty Tester','125-200% of Poverty Tester','Funding Code 4000 Tester','No Age for Client Tester','Untimely Closed Tester','Untimely Closed Overridden Tester','Citizenship & Immigration Tester','Active Advocate Tester','Retainer Tester','Caseworker Name','Compliance Check Untimely Closed','Compliance Check Untimely Closed Overwritten','Compliance Check 125 to 200 Poverty Income Ineligible','Compliance Check 200 Poverty Income Eligible','Compliance Check Citizenship and Immigration','Attestation on File?','Staff Verified Non-Citizenship Documentation','Did any Staff Meet Client in Person?','Close Reason','Date Closed','CSR: Is Legal Assistance Documented?','Age in Days','Percentage of Poverty','LSC Eligible?','CSR Eligible','Income Eligible','Primary Funding Codes','Secondary Funding Codes','DOB Information','Group','CSR: Timely Closing?','Was Timely Closed overridden?','Case Status','Level of Service','Retainer on File','Legal Problem Code','Primary Funding Codes','Secondary Funding Codes','Total Time For Case','Intake Type','Street Address','Assigned Branch/CC']]
        ...
        def save_xls(dict_df, path):
            ...
            for i in dict_df:
                ...
                worksheet.set_column('F:ZZ',0)
                worksheet.set_column('N:N',20)
                worksheet.set_column('P:P',20)
                worksheet.set_column('AN:AO',20)
                worksheet.set_column('AQ:AW',20)
