...
def ALComplianceConsolidater():
    ...
    if request.method == 'POST':
        ...
        def TesterTester(TwoHundredPercentTester,OneTwentyFivePercentTester,NoAgeTester,CitImmTester):
            if TwoHundredPercentTester != "" or OneTwentyFivePercentTester != "" or NoAgeTester != "" or CitImmTester != "":
                return "Needs Review"
            else:
                return ""
        df['Tester Tester'] = df.apply(lambda x : TesterTester(x['200% of Poverty Tester'],x['125-200% of Poverty Tester'],x['No Age for Client Tester'],x['Citizenship & Immigration Tester']),axis=1)        
        ...
        if request.form.get('Aggregate Data'):
            ...
            print("Split by Intake Caseworker")
        ...
        def save_xls(TabSplit, path):
            ...
            for splitval in tab_dict_df:
                ...
                B2AHFullRange='B2:AH'+str(tab_dict_df[splitval].shape[0]+1) 
                print("BAHRowRange is "+ str(B2AHFullRange)) 
                ...
                worksheet.conditional_format(B2AHFullRange,{'type': 'text',
                                                 'criteria': 'containing',
                                                 'value': 'Needs',
        ...
        return send_from_directory('sheets',f.filename, as_attachment = True, attachment_filename = "Comp Check " + f.filename) 
